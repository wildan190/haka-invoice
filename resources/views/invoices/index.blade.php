                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="{{ route('invoices.destroy', $invoice->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Hapus invoice ini?')">
    ...
    <div class="card mb-2 shadow-sm border-0 rounded-3">
        <div class="card-body">
            ...
            <h5 class="card-title mb-0">
                <a class="text-decoration-none d-block fw-bold text-dark" data-bs-toggle="collapse" href="#invoice-{{ $invoice->id }}" role="button">
                    Invoice: {{ $invoice->invoice_number }} <i class="fa-solid fa-chevron-down float-end"></i>
                </a>
            </h5>
            ...
            <div class="collapse mt-2" id="invoice-{{ $invoice->id }}">
                <p class="card-text text-muted small">
                    ...
                    <i class="fa-solid fa-user"></i> Rental: {{ $invoice->rental->customer->name }} - {{ $invoice->rental->mobil->merk }}
                </p>
                <div class="d-grid gap-2">
                    <a href="{{ route('invoices.show', $invoice->id) }}" class="btn btn-info btn-md w-100">
                        <i class="fa-solid fa-eye"></i> Lihat
                    </a>
                    <a href="{{ route('invoices.edit', $invoice->id) }}" class="btn btn-warning btn-md w-100">
                        <i class="fa-solid fa-pen"></i> Edit
                    </a>
                    <a href="{{ route('invoices.pdf', $invoice->id) }}" class="btn btn-danger btn-md w-100">
                        <i class="fa-solid fa-file-pdf"></i> PDF
                    </a>
                    <form action="{{ route('invoices.destroy', $invoice->id) }}" method="POST" class="d-grid">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-md w-100" onclick="return confirm('Hapus invoice ini?')">
                            <i class="fa-solid fa-trash"></i> Hapus
                        </button>
                    </form>
                </div>
