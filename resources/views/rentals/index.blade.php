                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form action="{{ route('rentals.destroy', $rental->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Hapus rental ini?')">
    ...
    <div class="card mb-2 shadow-sm border-0 rounded-3">
        <div class="card-body">
            ...
            <h5 class="card-title mb-0">
                <a class="text-decoration-none d-block fw-bold text-dark" data-bs-toggle="collapse" href="#rental-{{ $rental->id }}" role="button">
            ...
            <div class="collapse mt-2" id="rental-{{ $rental->id }}">
                <p class="card-text text-muted small">
                    <i class="fa-solid fa-car"></i> Mobil: {{ $rental->mobil->merk }} - {{ $rental->mobil->type }}<br>
                    <i class="fa-solid fa-clock"></i> Durasi: {{ $rental->duration }} {{ $rental->rental_type }}<br>
                    <i class="fa-solid fa-money-bill"></i> Total Harga: Rp{{ number_format($rental->total_price, 2, ',', '.') }}<br>
                ...
                <div class="d-grid gap-2">
                    ...
                    <form action="{{ route('rentals.destroy', $rental->id) }}" method="POST" class="d-grid">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-md w-100" onclick="return confirm('Hapus rental ini?')">
                            <i class="fa-solid fa-trash"></i> Hapus
                        </button>
                    </form>
                    @if ($rental->status == 'belum_lunas')
